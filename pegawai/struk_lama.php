...
// 1. AMBIL DATA PEMINJAMAN AKTIF TERLEBIH DAHULU
...
if ($peminjaman_saya) {
    // Format jam dari database harus 24 jam (HH:ii:ss)
    // Jika masih ada AM/PM, konversi ke 24 jam
    $jam_mulai_db = $peminjaman_saya['jam_mulai'];
    $jam_selesai_db = $peminjaman_saya['jam_selesai'];

    // Konversi ke format 24 jam jika diperlukan
    if (stripos($jam_mulai_db, 'pm') !== false || stripos($jam_mulai_db, 'am') !== false) {
        $jam_mulai_db = date("H:i:s", strtotime($jam_mulai_db));
    }
    if (stripos($jam_selesai_db, 'pm') !== false || stripos($jam_selesai_db, 'am') !== false) {
        $jam_selesai_db = date("H:i:s", strtotime($jam_selesai_db));
    }

    $mulai = strtotime($peminjaman_saya['tanggal_pinjam'] . ' ' . $jam_mulai_db);
    $selesai = strtotime($peminjaman_saya['tanggal_pinjam'] . ' ' . $jam_selesai_db);
    $batas_toleransi = $selesai + (15 * 60); // 15 menit toleransi

    // Debug logging
    error_log("DEBUG - Now: " . date('Y-m-d H:i:s', $now));
    error_log("DEBUG - Mulai: " . date('Y-m-d H:i:s', $mulai));
    error_log("DEBUG - Selesai: " . date('Y-m-d H:i:s', $selesai));
    error_log("DEBUG - Batas Toleransi: " . date('Y-m-d H:i:s', $batas_toleransi));

    // 2. CEK APAKAH SUDAH LEWAT BATAS TOLERANSI
    if ($now > $batas_toleransi) {
...
<body>
    ...
    <div class="main-content">
        ...
        <?php if ($peminjaman_saya && !$autoReset) { ?>
            <!-- STRUK PEMINJAMAN AKTIF -->
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="info-item">
                                ...
                                <span class="info-value">
                                    ...
                                    $jam_mulai_display = date("H:i", strtotime($peminjaman_saya['jam_mulai']));
                                    $jam_selesai_display = date("H:i", strtotime($peminjaman_saya['jam_selesai']));
                            ...
                            <div class="info-item">
                                ...
                                <span class="info-value">
                                    ...
                                    <?php } elseif ($status === 'Dikembalikan') { ?>
                                        <span class="badge badge-selesai status-badge">
                                ...
                                <div>
                                    ...
                                    <p class="mb-0">
                                        ...
                                        <strong><?= date('d/m/Y', strtotime($peminjaman_saya['tanggal_pinjam'])) ?>
                                            <?= date('H:i', strtotime($peminjaman_saya['jam_mulai'])) ?> -
                                            <?= date('H:i', strtotime($peminjaman_saya['jam_selesai'])) ?></strong>
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="action-buttons text-center mt-4">
                        <?php if ($status === 'Ditolak') { ?>
                            ...
                            <button class="btn btn-secondary" disabled>
                                <i class="fas fa-times-circle me-2"></i> Ditolak Admin
                            </button>
                        ...
                        <?php } elseif ($status === 'Menunggu Jadwal') { ?>
                            ...
                            <a href="cancel.php?id=<?= $peminjaman_saya['id']; ?>" class="btn btn-warning" onclick="return confirm('Yakin membatalkan peminjaman ini?')">
                        ...
                        <?php } elseif ($status === 'Lewat Batas') { ?>
                            ...
                            <a href="kembalikan.php?id=<?= $peminjaman_saya['id']; ?>" class="btn btn-danger">
                        ...
                        <?php } elseif ($status === 'Menunggu ACC') { ?>
                            ...
                            <a href="cancel.php?id=<?= $peminjaman_saya['id']; ?>" class="btn btn-warning" onclick="return confirm('Yakin membatalkan pengajuan peminjaman ini?')">
        ...
        <?php } else { ?>
            ...
            <div class="struk-card">
                <div class="struk-header">
                    <h3 class="mb-2">
                        <i class="fas fa-receipt me-2"></i>Struk Peminjaman
                    </h3>
                    <p class="mb-0">Status saat ini</p>
                </div>

                <div class="struk-body text-center py-5">
                    <div class="mb-4">
                        <i class="fas fa-car fa-4x text-muted mb-3"></i>
                        <h4 class="text-muted mb-3">Tidak Ada Peminjaman Aktif</h4>
        ...
        <footer class="mt-5 pt-3 text-center text-muted">
            ...
            <p class="mb-0">
                <small>Format waktu: 24 jam | Zona waktu: Asia/Makassar</small>
            </p>
        </footer>
    </div>
    ...
    <script>
        <?php if ($peminjaman_saya && ($status === 'Dipinjam' || $status === 'Lewat Batas')): ?>
            // COUNTDOWN TIMER - REAL TIME CLIENT SIDE
            // Konversi waktu dari PHP ke JavaScript
            const tanggalPinjam = "<?= $peminjaman_saya['tanggal_pinjam']; ?>";
            const jamSelesaiDB = "<?= $peminjaman_saya['jam_selesai']; ?>";

            // Pastikan format 24 jam
            let jamSelesaiClean = jamSelesaiDB;
            if (jamSelesaiDB.includes('PM') || jamSelesaiDB.includes('AM')) {
                // Jika ada AM/PM, konversi ke 24 jam di JavaScript
                const tempDate = new Date('1970-01-01 ' + jamSelesaiDB);
                jamSelesaiClean = tempDate.getHours().toString().padStart(2, '0') + ':' +
                    tempDate.getMinutes().toString().padStart(2, '0') + ':' +
                    tempDate.getSeconds().toString().padStart(2, '0');
            }

            const selesai = new Date(tanggalPinjam + 'T' + jamSelesaiClean + 'Z').getTime();
            const batasToleransi = selesai + (15 * 60 * 1000); // 15 menit dalam milidetik
            const countdownEl = document.getElementById("countdownTimer");

            function updateCountdown() {
                // 1. Ambil waktu sekarang dari sisi Client (Browser)
                const nowClient = new Date().getTime();

                // 2. Ambil waktu sekarang dari sisi Server (PHP)
                // Kita cetak nilai PHP time() ke dalam angka JavaScript
                const nowServer = <?= time(); ?> * 1000;

                // 3. Hitung selisihnya hanya satu kali di luar fungsi jika ingin efisien, 
                // tapi jika di dalam fungsi, pastikan logikanya seperti ini:
                const serverTimeOffset = nowServer - nowClient;

                // 4. Waktu yang sudah dikoreksi
                const correctedNow = nowClient + serverTimeOffset;

                // ... sisa kode logika countdown Anda menggunakan correctedNow ...
                const selesai = new Date("<?= $peminjaman_saya['tanggal_pinjam'] . ' ' . $peminjaman_saya['jam_selesai']; ?>").getTime();
                const diff = selesai - correctedNow;
                ...
                if (correctedNow >= batasToleransi) {
                    // Auto refresh untuk trigger auto return
                    countdownEl.innerHTML = '<span class="text-danger fw-bold">MOBIL AKAN DIKEMBALIKAN OTOMATIS</span>';
                    setTimeout(() => {
                ...
                if (correctedNow >= selesai) {
                    // Lewat batas waktu, hitung sisa toleransi
                    const diff = batasToleransi - correctedNow;
                    ...
                    document.title = `⏰ ${m}:${s} - Toleransi`;
                } else {
                    // Masih dalam waktu normal
                    const diff = selesai - correctedNow;
                    ...
                    document.title = `⏰ ${h}:${m}:${s} - Struk Peminjaman`;
                }
            }
        ...
        <?php elseif ($peminjaman_saya && $status === 'Menunggu ACC'): ?>
        ...
        function updateRealTimeClock() {
            ...
            // Update semua elemen dengan class .real-time-clock
